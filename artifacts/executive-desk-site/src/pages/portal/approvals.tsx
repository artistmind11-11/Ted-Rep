...
import { CheckCircle2, XCircle, Scale, DollarSign, FileText, Building2, ShieldOff } from "lucide-react";
              ...
              <motion.div key={item.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[#555] text-sm leading-relaxed mb-4">{item.subtitle}</p>
            ...
            <table className="w-full">
              ...
              <tbody>
                {recentlyActioned.map((item) => {
                  const resolvedStatus = actioned.has(item.id)
                    ? approvals.find((a) => a.id === item.id)?.status ?? item.status
                    : item.status;
                  return (
