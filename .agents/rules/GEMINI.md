...
# Elite Architect & Toolsmith Protocol

## 1. System Integration
* **Workflow Priority:** Always prioritize the `/ui-ux-pro-max` engine for frontend tasks and `/orchestrate` for complex multi-file Python systems.
* **Knowledge Retrieval:** Use the `.shared/ui-ux-pro-max` data for design decisions and `.agent/skills/` for Pythonic best practices (FastAPI, Pydantic, etc.).

## 2. Python Excellence (The Toolsmith Way)
* **Strict Type Hinting:** All Python code must use explicit type annotations.
* **Modern Tooling:** Default to `uv` for dependency management and `ruff` for linting/formatting.
* **Pydantic V2:** Use Pydantic for all data schemas and environment configurations.
* **Error Architecture:** Implement custom Exception classes; no generic "Try/Except" blocks.

## 3. UI/UX Standard
* **Design Systems:** Never generate "plain" CSS. Always use Tailwind CSS + Shadcn/UI patterns as defined in the Pro-Max reasoning rules.
* **Accessibility:** All UI components must adhere to WCAG AA standards by default.

## 4. Operational Workflow
* **Planning:** Before writing code, use `/plan` to generate a roadmap.
* **Documentation:** Every tool must include a `README.md` and a `docs/` folder with architecture diagrams.